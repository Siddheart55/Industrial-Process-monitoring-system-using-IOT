...
void esp01_connectAP()
{
	...
	UART0_Tx_str("AT+CWJAP=\"Rushi\",\"123456789\"\r\n");
	i=0;memset(buff,'\0',200);
	while(i<4);
	delay_ms(2500);
	buff[i] = '\0';
...
void esp01_sendToThingspeak(int val,int id)
{
	...
	if(strstr(buff,"CONNECT") || strstr(buff,"ALREADY CONNECTED"))
	{
		...
		//UART0_Tx_str("GET /update?api_key=AW2BIMVFK6IL3ZW2&field1=");
		...
		if(id==1)
		{
			...
			delay_ms(2000);
			UART0_Tx_str("GET /update?api_key=G2Y7UL1DKZFWRXO8&field1=");
		}
		else if(id==2)
		{
			//gas_value
			...
			delay_ms(2000);
			UART0_Tx_str("GET /update?api_key=G2Y7UL1DKZFWRXO8&field2=");
		}
...
void esp01_sendToThingspeak(int val, int id)
{
    ...
    if (strstr(buff, "CONNECT") || strstr(buff, "ALREADY CONNECTED"))
    {
        ...
        if (id == 1)  // Temperature
        {
            len = sprintf(request, "GET /update?api_key=MLPA4ZNHPU0VKDMA&field1=%d\r\n\r\n", val);
            Write_CMD_LCD(0x01);
            Write_str_LCD("Sending Temp");
        }
        else if (id == 2)  // Gas
        {
            len = sprintf(request, "GET /update?api_key=MLPA4ZNHPU0VKDMA&field2=%d\r\n\r\n", val);
            Write_CMD_LCD(0x01);
            Write_str_LCD("Sending Gas");
        }
